	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Oie👋🏻 @${num.split('@')[0]}\nMikasa su casa *${mdata.subject}*\nbem vindo leia as regras️`
				...
				teks = `by by @${num.split('@')[0]} nem faz falta msm🙄`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				success: '✔️ pronto, está ai mano ✔️',
				error: {
					stick: '⚠️ Falha, ocorreu um erro ao converter a imagem em sticker ⚠️',
					Iv: '❌ Link inválido ❌'
				},
				only: {
					group: '❌ só pode ser usado em grupos! ❌',
					ownerG: '⚠️ só pode ser usado pelo dono do bot! ⚠️',
			...
			switch(command) {
			    ...
			    case 'otagall':
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘 HELENA BOT 〙', members_id, true)
					break
                case 'marcar2':
					members_id = []
					teks = (args.le
                 ...
                 case 'marcar3':
					...
					if (args[0] === 'abrir') {
					    reply(`Grupo aberto genti`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'fechar') {
						reply(`Grupo fechado guys`)
        ...
        case 'promote':
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('Use o código do país anta')
					try {
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`banido ja era 😎 : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
                ...
                break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ tem que marcar um sticker ❌')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ Falha ao converter srickers em imagens ❌')
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('1 para ativar, 0 para desativar, lerdão você em 🤦🏻‍♀️')
					}
					break
				case 'welcome':
					if (!isGroup) return reply(mess.only.group)
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Já ativei')
						welkom.push(from)
